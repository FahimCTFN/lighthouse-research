...
import type {
  FilingStep,
  ShareholderVote as ShareholderVoteData,
  ShareholderVoteOutcome,
} from "@/lib/sanity/types";

const PARTY_LABEL: Record<string, string> = {
  target: "Target shareholders",
  acquirer: "Acquirer shareholders",
  both: "Both shareholder bases",
};
...
export function ShareholderVoteSection({
  votes,
}: {
  votes?: ShareholderVoteData[];
}) {
    ...
    <section className="rounded-lg border border-gray-200 bg-white">
      <header className="border-b border-gray-100 px-5 py-3">
        <h2 className="text-[11px] font-medium uppercase tracking-label text-gray-500">
          Shareholder Vote{votes.length > 1 ? "s" : ""}
        </h2>
      </header>
      <div className={votes.length > 1 ? "divide-y divide-gray-100" : ""}>
        {votes.map((vote, i) => (
          <VoteCard key={vote._key ?? i} vote={vote} />
        ))}
      </div>
    </section>
  );
...
function VoteCard({ vote }: { vote: ShareholderVoteData }) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <h3 className="text-[13px] font-semibold text-brand-navy">
            {PARTY_LABEL[vote.party ?? "target"] ?? "Shareholders"}
          </h3>
          {vote.label && (
            <p className="mt-0.5 text-[12px] text-gray-600">{vote.label}</p>
          )}
        </div>
        <span
...
function CommittedMeter({
  ...
  return (
    <div className="mt-3 rounded-md border border-brand-gold/30 bg-brand-gold-light/60 px-4 py-3">
